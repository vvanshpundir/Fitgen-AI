...
from fitgenserver.ai_trainer_model.DeepFitClassifier import DeepFitClassifier
import numpy as np
...
def set_video_feed_variables():
    ...
    model_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),  # Get the directory of this script
        'models',  # Subdirectory
        'deepfit_classifier_v3.tflite'  # Model file
    )
    clf = DeepFitClassifier(model_path)
...
def main():
    ...
    user_input = input("Do you want to upload a video (Y/N)? ").strip().upper()

    if user_input == 'Y':
        # File dialog for video file selection
        root = tk.Tk()
    ...
    cv2.destroyAllWindows()
'''
def save_reps_to_csv(reps, filepath='reps.csv'):
...
def main(video_path):
    ...
    cap = cv2.VideoCapture(video_path)  # Load the video from the path
    ...
    return {
        "count": count,
        "feedback": feedback,
        "workout_type": workout_name_after_smoothening
    }
